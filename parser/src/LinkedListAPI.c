...
char* toString(List * list){
	...
	while(currentNode != NULL){
		char* currDescr = list->printData(currentNode->data);
...
void clearList(List* list){
	if(list == NULL){
        //printf("List is not initialized. Initialize by calling List *initializeList(void (*printFunction)(void *toBePrinted),void (*deleteFunction)(void *toBeDeleted),int (*compareFunction)(const void *first,const void *second))\n");
        return;
...
ListIterator createIterator(List *list){
	...
	if(list->head == NULL){
		printf("create Iterator fucntion: List is not initialiezed.\n");
	}

	ListIterator itera;
	itera.current = list->head;
...
void* nextElement(ListIterator* iter){
	...
	Node * iterNext = iter->current;
	iter->current = iter->current->next;
	return iterNext->data;
}
